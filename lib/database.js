...
class Database {
  ...
  async connect() {
    ...
    /**
     * After a connection error, the client is invalid, and simply reconnecting
     * is not possible. So if we run into an error, let's reconnect.
     */
    this._client.on("error", (error) => {
      console.exception("Postgres connection failed.");
      console.exception(error);
      console.exception("Attempting to create a new connection...");
      this.connect();
    });
  ...
  async getHistoricalData(startDate, endDate, distribution) {
    let result = await this._client.query(
      "SELECT datetime, counters -> $3 as counters FROM history WHERE datetime BETWEEN $1 AND $2 ORDER BY datetime ASC;",
